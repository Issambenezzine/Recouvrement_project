...
const getDebiteurAddresse = async (req, res) => {
  try {
    const { debiteurId,marche } = req.body;
    const addresse = await Debiteur_Addresse.findAll({ where: { marche:marche, debiteurId:debiteurId } });
    res.status(200).json(addresse);
  } catch (err) {
    console.log(`Error getting Debiteur Addresse : ${err.message}`);
  }
}


const getGestionDebiteur = async (req,res) => {
  try {
