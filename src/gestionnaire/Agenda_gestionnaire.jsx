...
import CardeGestionnaire from "../gestionnaire/Carde_gestionnaire";
...
export default function Agenda_gestionnaire() {
  ...
  const verifyUser = async () => {
    try {
      ...
      const res = await axios.get(
        'http://localhost:3004/auth/gestionnaire',
  ...
  const handleCardClick = async (tableType) => {
    console.log(tableType)
    let idGest = localStorage.getItem("UserId")
    
    if (tableType === "Nouvelles ations") {
      setIsLoading(true); // Start loading
      
      try {
        const res = await axios.post("http://localhost:3004/gestionnaire/valider",
          { idGest },
          { withCredentials: true }
        )
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", backgroundColor: "#f3f4f6" }}>
      ...
      <main style={{ flex: 1, padding: "1rem 2rem", overflow: "hidden" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem", height: "100%" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: "1rem" }}>
            <CardeGestionnaire onCardClick={handleCardClick} dossiers={data} onNewClick={() => setShowNewPopup(true)} />
          </div>
          ...
          <div style={{ display: "flex", gap: "1rem", flex: 1, minHeight: 0 }}>
            ...
            <section style={{ width: "80%", height: "100%", overflowY: "auto", boxShadow: "0px 0px 10px rgba(0,0,0,0.1)" }}>
              <div style={{ height: "100%", border: "1px solid #e5e7eb", borderRadius: "6px" }}>
                <table style={{ width: "100%", minWidth: "900px", borderCollapse: "collapse", backgroundColor: "#fff" }}>
                  ...
                  <tbody style={{ fontSize: "0.75rem" }}>
                    {filteredData.map((row, index) => (
                      <tr
                        key={index}
                        style={{
                          backgroundColor: index % 2 === 0 ? "#f1f5f9" : "#ffffff",
                          cursor: "pointer",
                        }}
                        onClick={() => handleRowClick(row)}
                      >
                        <td style={tdStyle}><FaInfoCircle style={{ color: "#2563eb" }} /></td>
      ...
      {showDossier && selectedClient && (
        <GestionDossier client={selectedClient} onClose={() => setShowDossier(false)} dossiers={data}/>
      )}
