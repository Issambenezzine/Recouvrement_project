...
// function Card({ label, count, amount, bgColor, color, active, icon: Icon, onClick }) {
//   return (
//     <div
//       onClick={onClick}
//       style={{
...
//         <div style={{ fontSize: '12px', fontWeight: "bold", marginBottom: '2px' }}>{label}</div>
//         <div style={{ fontSize: '16px', fontWeight: "bold" }}>{count}</div>
//         <div style={{ fontSize: '11px', opacity: 0.9 }}>{amount}</div>
//       </div>
//     </div>
//   );
// }

// export default function CardeGestionnaire({ onCardClick, onNewClick }) {
//   const [activeCard, setActiveCard] = useState(null);

//   const cards = [
//     {key: "new",label: "Nouveau",count: "",amount: "",bgColor: "#9f8081ff",color: "#fff",icon: FaPlusCircle },
//     {key: "aTraiter",label: "Dossiers à traiter",count: "188",amount: "7 905 953.94 MAD",bgColor: "#470505e1",color: "#fff", icon: FaPlusCircle },
//     {key: "backlog",label: "En backlog",count: "6374",amount: "253 201 962.21 MAD",bgColor: "#c29393ff",color: "#fff",icon: FaFolder },
//     {key: "enCours", label: "En cours",count: "2459",amount: "110 770 312.76 MAD",bgColor: "#908d8ddf",color: "#fff",icon: FaFolder },
//     {key: "suiteCadrage",label: "Suite au cadrage",count: "1021",amount: "22 855 640.76 MAD",bgColor: "#9f8081ff",color: "#fff",icon: FaFolder },
  ...
  useEffect(() => {
    const countStats = () => {
      const stats = states.map((state) => {
        const filtered = dossiers.filter((elem) => elem.dossier.etat === state);
        console.log("dossier in card : ", dossiers);
