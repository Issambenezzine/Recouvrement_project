...
const Action = ({ actions, onAddActionn, dossier }) => {
  ...
  const fetchActions = async () => {
    try {
      const res = await axios.post(
        "http://localhost:3004/action/get",
  ...
  const handleAdd = async (newAction) => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:3004/action/save",
        { newAction, idManager: dossier.dossier.responsableId, idGest: dossier.dossier.gestionnaireId },
        {
          withCredentials: true,
        }
      );
            ...
            {actions.length === 0 ? (
              ...
              action.map((action, index) => (
                ...
                >
                  ...
                  <td style={{ padding: "8px" }}>{action.datePrevue || "-"}</td>
                  <td style={{ padding: "8px" }}>
