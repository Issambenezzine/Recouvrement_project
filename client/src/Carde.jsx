  ...
  useEffect(() => {
    const countStats = () => {
      const stats = states.map((state) => {
        let filtered = []
        if (state === "Archive") {
          filtered = dossiers.filter((elem) => elem.dossier.gestionnaireId === null || elem.dossier.manager === null);
        
        }else if(state === "Actions suite au cadrage") {
          filtered = dossiers.filter((elem) => elem.dossier.cadrage === "OUI")
