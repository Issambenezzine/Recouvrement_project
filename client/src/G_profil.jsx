  ...
  const fetchUsers = async () => {
    try {
      const res = await axios.get(
        `http://${HOST}:${PORT}/user/users/${localStorage.getItem("UserId")}`
      );
      setProfile(res.data);
    } catch (err) {
  ...
  const confirmDelete = async () => {
    try {
      const res = await axios.delete(
        `http://${HOST}:${PORT}/${deleteUserRole.toLowerCase()}/delete`,
        {
  ...
  const handleUpdateUser = async (updatedUser) => {
    try {
      const res = await axios.put(
        `http://${HOST}:${PORT}/${updatedUser.role.toLowerCase()}/update`,
        updatedUser,
        {
          withCredentials: true,
          headers: {
        ...
        <h2>
          ...
          {localStorage.getItem("Role") === "ADMIN" && (
            <FaPlus
              style={{
                cursor: "pointer",
                color: hoveredPlus ? "green" : "#020144ff",
                width: 40,
              }}
              onClick={handleAdd}
        ...
        <tbody>
          {filteredProfiles.map((profile, index) => (
            <tr key={index} style={{ textAlign: "center", height: "50px" }}>
              ...
              <td>{profile.objectif ? profile.objectif + " MAD" : "__"}</td>
              <td
                style={{
                  fontWeight: "bold",
