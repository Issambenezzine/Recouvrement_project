...
export default function Agenda_manager({ data }) {
  ...
  const verifyUser = async () => {
    try {
      ...
      const res = await axios.get(`http://${HOST}:${PORT}/auth/gestionnaire`, {
  ...
  const handleCardClick = async (tableType) => {
    ...
    if (tableType === "Nouvelles actions") {
      setIsLoading(true); // Start loading

      try {
        const res = await axios.post(
          `http://${HOST}:${PORT}/gestionnaire/valider`,
          { idGest },
          { withCredentials: true }
        );
  ...
  const exportToExcel = () => {
    ...
    filteredData.forEach((row) => {
      exportData.push({
        ...
        "Débiteur Date Naissance": row.debiteurInfo.debiteur.date_naissance,
